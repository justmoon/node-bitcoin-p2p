...
PeerManager.prototype.connectTo = function (peer)
{
	...
	var that = this;
	peer.createConnection(function(error, socket){
		if(!error) {
			var conn = new Connection(that.node, socket, peer);
			that.connections.push(conn);
			that.node.addConnection(conn);

			conn.addListener('verack', that.handleReady.bind(that));
			conn.addListener('error', that.handleError.bind(that));
			conn.addListener('end', that.handleDisconnect.bind(that));
		}
		else {
			logger.log('error','Error creating connection',error);
		}
	});
