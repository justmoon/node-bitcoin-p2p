...
var BlockChain = exports.BlockChain = function (storage) {
	...
	this.addTransactions = function addTransactions(block, txs) {
		txs.forEach(function (tx, i) {
			var tx = new Transaction(txs[i]); //FIXME tx is parameter of callback function
	...
	this.reorganize = function reorganize(oldTopBlock, newTopBlock) {
		...
		function findFork(bOld, bNew, callback) {
			if (bOld.hash.compare(bNew.hash) === 0) {
				callback(null, bOld);
			}

			if (bOld.height > bNew.height) {
				toDisconnect.push(bOld);
				self.getBlockByPrev(bOld, function (err, bOld) {
					if (err) {
						callback(err);
						return;
					}
					findFork(bOld, bNew, callback);
				});
			} else {
				if (bNew.height <= 0) {
				...
				toConnect.push(bNew);
				self.getBlockByPrev(bNew, function (err, bNew) {
					if (err) {
						callback(err);
						return;
					}
		...
		findFork(oldTopBlock, newTopBlock, function (err) {
			if (err) {
