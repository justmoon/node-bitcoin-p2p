...
Node.prototype.handleTx = function (e) {
	...
	this.txStore.add(tx, function (err) {
		if (err) {
			logger.warn("Rejected transaction " +
						Util.formatHash(tx.getHash()) + " " +
						err.message);
		} else {
			logger.info("Added transaction " + Util.formatHash(tx.getHash()));
		}
	});
};
...
Node.prototype.sendTx = function (tx, callback) {
	this.txStore.add(tx, (function (err, fees) {
		if (!err) {
