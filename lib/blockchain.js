	...
	this.addTransactions = function addTransactions(block, txs) {
		txs.forEach(function (tx, i) {
			...
			tx.save(function (err) {
				...
				// We'll mark transactions as spent asynchronously
				// TODO: Maintain a list of these saves in progress as otherwise
				//       there is a, albeit short, window of time where a double
				//       spend could pass validation.
				tx.ins.forEach(function (txin) {
					if (txin.isCoinBase()) return;
					var setDef = {};
					setDef['outs.'+txin.outpoint.index+'.spent'] = true;
					Transaction.collection.update( { hash: txin.outpoint.hash.toString('base64') }, { $set: setDef }, function (err) {
						if (err) logger.warn("Error while marking transaction as spent", err);
					});

					self.emit('txSpend', {
						to: {block: block, tx: tx, index: i},
